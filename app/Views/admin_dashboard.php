        ...
        <script>
            ...
            function updateFolderName(input) {
                if(input.files && input.files.length > 0) {
                    // 1. Get the path of the first file (e.g., "MyFolder/sub/file.txt")
                    let fullPath = input.files[0].webkitRelativePath;
                    // 2. Extract the root folder name ("MyFolder")
                    let rootFolder = fullPath.split('/')[0];
                    
                    // 3. Set it to the hidden input so PHP receives it
